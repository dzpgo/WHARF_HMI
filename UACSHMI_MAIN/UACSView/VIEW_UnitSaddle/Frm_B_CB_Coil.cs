...
namespace UACSView
{
    public partial class Frm_B_CB_Coil : FormBase
    {
        public Frm_B_CB_Coil()
        {
            InitializeComponent();
            SetStyle(ControlStyles.AllPaintingInWmPaint
                | ControlStyles.OptimizedDoubleBuffer
                | ControlStyles.UserPaint, true);
            this.Load += Frm_B_CB_Coil_Load;
        }
        #region  -----------------------------连接数据库--------------------------------
        ...
        #region  -----------------------------方法字段--------------------------------
        private const string BayNo = "B";
        private const string AreaNo_B = "Z05-D";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {              
                string sql1 = @"UPDATE UACS_OFFLINE_PACKING_AREA_STOCK_DEFINE SET CONFIRM_FLAG = 130 WHERE STOCK_NO IN (SELECT STOCK_NO FROM UACS_YARDMAP_STOCK_DEFINE WHERE MAT_NO != 'NULL' AND STOCK_STATUS = 2 AND LOCK_FLAG = 0 AND STOCK_NO IN (SELECT STOCK_NO FROM UACS_OFFLINE_PACKING_AREA_STOCK_DEFINE WHERE AREA_ID = '"+ AreaNo_B + "'))";
